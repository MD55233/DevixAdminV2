...
const TrainingBonusApproved = () => {
  const [transactions, setTransactions] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    const fetchApprovedTrainingBonuses = async () => {
      try {
        // Fetch only approved training bonuses
        const response = await axios.get(`${process.env.REACT_APP_API_HOST}/api/approvals/approve`);
        ...
        const combinedTransactions = response.data.map(item => ({
          ...
          remarks: 'Training bonus approved', // Set a static remark or adjust based on your logic
        }));
    ...
    <Grid container spacing={3} justifyContent="center">
      <Grid item xs={12}>
        <Typography variant="h3" gutterBottom sx={{ color: 'secondary.main', textAlign: 'center' }}>
          Training Bounus Approved History 
        </Typography>
      </Grid>
...
export default TrainingBonusApproved;
